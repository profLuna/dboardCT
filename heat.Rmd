...
# rm(ct_rural_sf, ct_urban_sf)
# ct_blkgrpLST_sf <- ct_blkgrpLST_sf %>% 
#   select(!(OBJECTID_15:Shape_Area))

ct_blkgrpLST_sf <- readRDS(file = "../DATA/ct_blkgrps_sf_CUM.Rds")
...
ct_blkgrpLST_sf <- county_subdivisions("CT") %>% 
  st_transform(., crs = st_crs(ct_blkgrpLST_sf)) %>% 
  transmute(TOWN = NAME) %>% 
  st_join(ct_blkgrpLST_sf, ., largest = TRUE) %>% 
...
ct_blkgrpLST_sf <- house_districts %>% 
  select(REP_DIST) %>% 
  st_join(ct_blkgrpLST_sf, ., largest = TRUE)

# Assign state senate district names to block groups
ct_blkgrpLST_sf <- senate_districts %>% 
  select(SEN_DIST) %>% 
  st_join(ct_blkgrpLST_sf, ., largest = TRUE)
...
muniDayNight_80th <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanAvgLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>%
  group_by(TOWN) %>% 
  summarize(BGs80thDayNight = n())

houseDayNight_80th <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanAvgLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>%
  group_by(REP_DIST) %>% 
  summarize(BGs80thDayNight = n())

senateDayNight_80th <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanAvgLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>%
  group_by(SEN_DIST) %>% 
  summarize(BGs80thDayNight = n())
...
muniUHI24h_80th <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(UHI24avg) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thUHI24h = n())

houseUHI24h_80th <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(UHI24avg) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thUHI24h = n())

senateUHI24h_80th <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(UHI24avg) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thUHI24h = n())
...
muniNight_80th <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanNightLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thNight = n())

houseNight_80th <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanNightLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thNight = n())

senateNight_80th <- ct_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanNightLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thNight = n())
...
# create layer of ct_blkgrpLST_sf for 80th percentile
ct_blkgrps_sf_DayNight <- ct_blkgrpLST_sf %>% 
...
ct_blkgrps_sf_UHI24h <- ct_blkgrpLST_sf %>% 
...
ct_blkgrps_sf_Night <- ct_blkgrpLST_sf %>% 
...
LST24hrPop <- ct_blkgrpLST_sf %>% 
  ...
  filter(Group %in% c("totalpopE","minorityE","eng_limitE","num2povE","lthsE","under5E","over64E","CT_LOWINC")) %>% 
...
# clean up
rm(list = ls(pattern = paste("muni", "_80th", "houseN", "senateN", "houseL", "senateL", "houseU", "senateU", "houseD", "senateD", "ct_blkgrpLST_sf$", sep = "|")))
```
---
# Heat
...
* Higher temperatures throughout the day and night are found in all urbanized areas of the state and follow the major roadways very closely. This is also true for areas experiencing high levels of urban heat island effect. 
* Limited English speaking households and Connecticut’s low income EJ block groups are more likely to experience the highest daytime and nighttime temperatures than the general population.

These interactive figures identify communities across Connecticut that are most overburdened and most vulnerable to three measures of ambient temperature: Day-Night average land surface temperatures (LST), the Urban Heat Island effect, and elevated nighttime LST.
...
pal <- colorQuantile(
  palette = "YlOrRd",
  domain = ct_blkgrps_sf_DayNight$meanAvgLST,
  n = 5)
...
PopupHouse <- paste0("Connecticut state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing Day-Night average land surface temperatures at the 80th percentile - ", round(min(ct_blkgrps_sf_DayNight$meanAvgLST),1),"° - or higher.", "<br/>",
...
PopupSenate <- paste0("Connecticut state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing Day-Night average land surface temperatures at the 80th percentile - ", round(min(ct_blkgrps_sf_DayNight$meanAvgLST),1),"° - or higher.", "<br/>",
...
Popup <- paste0(ct_blkgrps_sf_DayNight$NAME, "<br/>",
                "<b>State House District:</b> ", ct_blkgrps_sf_DayNight$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ct_blkgrps_sf_DayNight$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ct_blkgrps_sf_DayNight$TOWN, "<br/>",
                      "<b>Day-Night Avg Temp (°F):</b> ", round(ct_blkgrps_sf_DayNight$meanAvgLST,1),"°", "<br/>",
         "<b>Day-Night Avg Temp percentile:</b> ", round(ct_blkgrps_sf_DayNight$meanAvgLSTpctile*100,0), "<br/>",
                      "<b>Priority Populations: </b>", ct_blkgrps_sf_DayNight$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = ct_blkgrps_sf_DayNight, 
  ...
  addLegend(title = "Highest Day-Night<br>Avg LST (°F) and<br>High Priority Pops", 
            pal = pal,
            values = ct_blkgrps_sf_DayNight$meanAvgLST,
            position = "bottomleft",
...
pal <- colorQuantile(
  palette = "YlOrRd",
  domain = ct_blkgrps_sf_UHI24h$UHI24avg,
  n = 5)
...
PopupHouse <- paste0("Connecticut state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing Day-Night average urban heat island effects at the 80th percentile - ", round(min(ct_blkgrps_sf_UHI24h$UHI24avg),1),"° - or higher.", "<br/>",
...
PopupSenate <- paste0("Connecticut state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing Day-Night average urban heat island effects at the 80th percentile - ", round(min(ct_blkgrps_sf_UHI24h$UHI24avg),1),"° - or higher.", "<br/>",
...
Popup <- paste0(ct_blkgrps_sf_UHI24h$NAME, "<br/>",
                "<b>State House District:</b> ", ct_blkgrps_sf_UHI24h$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ct_blkgrps_sf_UHI24h$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ct_blkgrps_sf_UHI24h$TOWN, "<br/>",
                      "<b>Day-Night Avg UHI (°F):</b> ", round(ct_blkgrps_sf_UHI24h$UHI24avg,1),"°", "<br/>",
         "<b>Day-Night Avg UHI percentile:</b> ", round(ct_blkgrps_sf_UHI24h$UHI24avgpctile*100,0), "<br/>",
                      "<b>Priority Populations: </b>", ct_blkgrps_sf_UHI24h$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ct_blkgrps_sf_UHI24h, 
              color = ~pal(UHI24avg),
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
  ...
  addLegend(title = "Highest Day-Night<br>Urban Heat Island<br>effects (°F) and<br>High Priority Pops", 
            pal = pal,
            values = ct_blkgrps_sf_UHI24h$UHI24avg,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
pal <- colorQuantile(
  palette = "YlOrRd",
  domain = ct_blkgrps_sf_Night$meanNightLST,
  n = 5)
...
PopupHouse <- paste0("Connecticut state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing nighttime average land surface temperatures at the 80th percentile - ", round(min(ct_blkgrps_sf_Night$meanNightLST),1),"° - or higher.", "<br/>",
...
PopupSenate <- paste0("Connecticut state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing nighttime average land surface temperatures at the 80th percentile - ", round(min(ct_blkgrps_sf_Night$meanNightLST),1),"° - or higher.", "<br/>",
...
Popup <- paste0(ct_blkgrps_sf_Night$NAME, "<br/>",
                "<b>State House District:</b> ", ct_blkgrps_sf_Night$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ct_blkgrps_sf_Night$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ct_blkgrps_sf_Night$TOWN, "<br/>",
                      "<b>Night Avg Temp (°F):</b> ", round(ct_blkgrps_sf_Night$meanNightLST,1),"°", "<br/>",
         "<b>Night Avg Temp percentile:</b> ", round(ct_blkgrps_sf_Night$meanNightLSTpctile*100,0), "<br/>",
                      "<b>Priority Populations: </b>", ct_blkgrps_sf_Night$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ct_blkgrps_sf_Night, 
  ...
  addLegend(title = "Highest Nighttime<br>Avg LST (°F) and<br>High Priority Pops", 
            pal = pal,
            values = ct_blkgrps_sf_Night$meanNightLST,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
[Moderate-resolution Imaging Spectroradiometer (MODIS)](https://modis.gsfc.nasa.gov/data/dataprod/mod11.php) satellite sensor. The three specific heat metrics highlighted here include: day-night average LST in the 80th percentile (i.e., `r round(min(ct_blkgrps_sf_DayNight$meanAvgLST),1)`°F or warmer), day-night average urban heat island effects in the 80th percentile (i.e., `r round(min(ct_blkgrps_sf_UHI24h$UHI24avg),1)`°F or warmer), and nighttime average LST in the 80th percentile (i.e., `r round(min(ct_blkgrps_sf_Night$meanNightLST),1)`°F or warmer). These metrics are three of the six heat metrics used to calculate [cumulative Heat burdens](index.html).
...
### Day-Night Avg LST
```{r graphDayNight, fig.align="center", fig.cap="Population-weighted average exposures to day-night average temperatures for populations of concern in Connecticut."}
...
### Urban Heat Island
```{r graphUHI, fig.align="center", fig.cap="Population-weighted average exposures to day-night average temperatures for populations of concern in Connecticut."}
# Add status variable and recode demographic names
UHI24hrPop %>% 
  mutate(
    Status = case_when(
...
### Nighttime Avg LST
```{r graphNight, fig.align="center", fig.cap="Population-weighted average exposures to nighttime average temperatures for populations of concern in Connecticut."}
# Add status variable and recode demographic names
LSTNightPop %>% 
  mutate(
    Status = case_when(
...
# create variables for use in inline text example to explain percentile ranks
exLSTtown <- ct_towns %>% as.data.frame() %>% filter(pctrank_meanAvgLST.mean > 95) %>% select(NAME) %>% slice_tail(n = 1) %>% pull()
exLST <- ct_towns %>% as.data.frame() %>% filter(NAME == exLSTtown) %>% select(meanAvgLST.mean) %>% pull() %>% round(., 1)
exLSTpct <- ct_towns %>% as.data.frame() %>% filter(NAME == exLSTtown) %>% select(pctrank_meanAvgLST.mean) %>% pull() %>% round(., 1)
...
These tables show the average day-night land surface temperatures (LST), Urban Heat Island (UHI) effects, and average nighttime LST by jurisdiction for July-August 2019. LST values are averaged across Census block groups within the jurisdiction. A rank indicates the percentage of jurisdictions that are below that value. For example, `r exLSTtown` had a Day-Night average Land Surface Temperature (LST) of `r exLST`° Fahrenheit, and a Day-Night LST rank of `r exLSTpct`. The latter means that `r exLSTtown`'s average LST was higher than `r exLSTpct`% of all other municipalities in the state.
