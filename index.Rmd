...
meta() %>%
  meta_description(
    "Environmental justice analysis of transportation-related burdens and benefits in Connecticut") %>% 
  meta_viewport() %>% 
  meta_social(
    title = "Investing for Equity in Connecticut",
...
house_districts <- st_read("../DATA/shapefiles/housect_37800_0000_2010_s100_census_1_shp/nad83","housect_37800_0000_2010_s100_census_1_shp_nad83_meters") %>% 
  ...
  st_transform(., crs = 4326) %>% 
  st_make_valid() %>% 
  left_join(., read_csv("../tables/CT_burdensCnt_house.csv"), 
...
senate_districts <- st_read("../DATA/shapefiles/senatect_37800_0000_2010_s100_census_1_shp/nad83","senatect_37800_0000_2010_s100_census_1_shp_nad83_meters") %>% 
  ...
  st_transform(., crs = 4326) %>% 
  st_make_valid() %>% 
  left_join(., read_csv("../tables/CT_burdensCnt_senate.csv"), 
...
# reduce number of variables in ct_blkgrps_sf for faster loading AND reduce number of vertices
ct_blkgrps_sf <- ct_blkgrps_sf %>%
...
* Areas with high percentages of priority populations and the highest rankings of multiple cumulative environmental burdens fall along I-95 in the southeast corner of the state from Greenwich to New Haven, and in the greater Hartford and New Britain region. 
* People living in Connecticut's state-designated low income communities, limited English speaking households, and people of color are the most affected groups, especially for air quality, heat, and flood or evacuation risk. 
* Priority populations, especially those over 64, living in rural and suburban areas, lack transportation options and face high costs for transportation.
...
pal <- colorFactor(
  palette = "YlOrRd",
  domain = ct_blkgrps_sf$BurdenCountLabel)
...
PopupHouse <- paste0("Connecticut state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$`3 Burdens`,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing <b>3 categories</b> of highest cumulative burdens, and ", "<b>",house_districts$`4 Burdens`,"</b>", " <b>Block Groups</b> experiencing <b>4 categories</b> of highest cumulative burdens.")

PopupSenate <- paste0("Connecticut state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$`3 Burdens`,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing <b>3 categories</b> of highest cumulative burdens, and ", "<b>",senate_districts$`4 Burdens`,"</b>", " <b>Block Groups</b> experiencing <b>4 categories</b> of highest cumulative burdens.")

Popup <- paste0(ct_blkgrps_sf$NAME, "<br/>",
                "<b>State House District:</b> ", ct_blkgrps_sf$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ct_blkgrps_sf$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ct_blkgrps_sf$TOWN, "<br/>",
                      "<b>Number of Burdens:</b> ", ct_blkgrps_sf$BurdenCount, "<br/>",
                      "<b>Burden Categories: </b>", ct_blkgrps_sf$BURDENSlabel,"<br/>",
                      "<b>Priority Populations: </b>", ct_blkgrps_sf$POPSlabel)

leaflet(width = "100%") %>% 
  addProviderTiles(providers$Stamen.TonerLite) %>% 
  addPolygons(data = ct_towns,
              weight = 0.7,
              opacity = 1,
              color = "gray",
              fillOpacity = 0,
  ...
  addPolygons(data = ct_blkgrps_sf,
  ...
  addLegend(title = "Cumulative Burdens", 
            ...
            values = ct_blkgrps_sf$BurdenCountLabel,
...
ct_cum_burdens <- read_csv("../tables/CT_cum_burden.csv") %>% 
  as.data.frame() %>% 
...
LEH1 <- ct_cum_burdens %>% filter(Group == "CT Low Income") %>% select(`1 Burden`) %>% pull()
LEH2 <- ct_cum_burdens %>% filter(Group == "CT Low Income") %>% select(`2 Burdens`) %>% pull()
LEH4 <- ct_cum_burdens %>% filter(Group == "CT Low Income") %>% rowwise %>% summarize(Total = sum(c_across(2:5))) %>% pull()
THH <- ct_cum_burdens %>% filter(Group == "Total Pop") %>% rowwise %>% summarize(Total = sum(c_across(2:5))) %>% pull()
...
categories_column <- names(ct_cum_burdens[,1])
measure_columns <- names(ct_cum_burdens[,2:5])

stacked_bar_chart(data = ct_cum_burdens,
...
For example, `r LEH1`% of people living in areas designated by state policy as low income (i.e., 'CT Low Income') experience 1 high environmental burden category, and `r LEH2`% reside in block groups experiencing 2 environmental burden categories. Cumulatively, `r LEH4`% of CT Low Income residents live in communities experiencing 1 to 4 highest environmental burden categories. Compare this to the `r THH`% of the general population in the state (i.e., 'Total Pop') who experience these conditions. 
...
read_csv("../tables/CT_burdens_town.csv") %>% 
...
read_csv("../tables/CT_burdens_house.csv") %>% 
  mutate(across(starts_with("Pct"), ~./100)) %>% # let DT do formatting
  datatable(., rownames = FALSE, options = list(pageLength = 10), 
            container = sketch2) %>% 
  formatRound(., columns = c(2,4,6), digits = 0, mark = ",") %>% 
...
read_csv("../tables/CT_burdens_senate.csv") %>% 
  mutate(across(starts_with("Pct"), ~./100)) %>% # let DT do formatting
  datatable(., rownames = FALSE, options = list(pageLength = 10), 
            container = sketch3) %>% 
  formatRound(., columns = c(2,4,6), digits = 0, mark = ",") %>% 
