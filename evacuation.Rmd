...
ct_blkgrps_sf <- ne_blkgrp_sf %>% 
  ...
  select(GEOID, NAME, STATE, bg_area_m2, totalpopE, minorityE, minority_pctE, under5E, pct_under5E, under18E, pct_under18E, over64E, pct_over64E, householdsE, eng_hhE, eng_limitE, eng_limit_pctE, age25upE, lthsE, pct_lthsE, povknownE, num2povE, pct2povE, CT_INCOME) %>%
...
NFHZAbyTown <- ct_blkgrps_nfhza %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(st_transform(ct_towns, st_crs(ct_blkgrps_nfhza))) %>% 
...
# Calculate Pop and Pct pop at risk by jurisdiction
HEVACbyTown <- ct_blkgrps_hevac %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(st_transform(ct_towns, st_crs(ct_blkgrps_hevac))) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
house_names_pops <- get_acs(geography = "block group", survey = "acs5",
                     variables = c(totalpop = "B03002_001"), state = "CT", 
                     output = "wide", year = 2018, geometry = TRUE) %>% 
  select(GEOID, totalpopE) %>% 
  st_transform(.,st_crs(ct_blkgrps_nfhza)) %>% 
  select(totalpopE) %>% 
  mutate(OldArea = as.numeric(st_area(.))) %>% 
    st_intersection(
      st_make_valid(
        st_transform(house_districts,st_crs(ct_blkgrps_nfhza))
        )
      ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
house_floodPop_pct <- ct_blkgrps_hevac %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
      st_transform(house_districts, st_crs(ct_blkgrps_hevac))
      )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
senate_names_pops <- get_acs(geography = "block group", survey = "acs5",
  ...
  mutate(OldArea = as.numeric(st_area(.))) %>% 
    st_intersection(
      st_make_valid(
        st_transform(senate_districts,st_crs(ct_blkgrps_nfhza))
        )
      ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
# identify FEMA risk pops by district and calculate pop pct
NFHZAbySenate <- ct_blkgrps_nfhza %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
      st_transform(senate_districts, st_crs(ct_blkgrps_nfhza))
...
# identify HEVAC risk pops by district and calculate pop pct, and bring together with FEMA
senate_floodPop_pct <- ct_blkgrps_hevac %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
      st_transform(senate_districts, st_crs(ct_blkgrps_hevac))
      )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
muniNFHZA <- ct_blkgrps_sf %>%
  as.data.frame() %>%
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>%
  group_by(TOWN) %>%
  summarize(BGsNFHZA = n())

houseNFHZA <- ct_blkgrps_sf %>%
  as.data.frame() %>%
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>%
  group_by(REP_DIST) %>%
  summarize(BGsNFHZA = n())

senateNFHZA <- ct_blkgrps_sf %>%
  as.data.frame() %>%
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>%
  group_by(SEN_DIST) %>%
  summarize(BGsNFHZA = n())
...
muniHEVAC <- ct_blkgrps_sf %>%
  as.data.frame() %>%
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>%
  group_by(TOWN) %>%
  summarize(BGsHEVAC = n())

houseHEVAC <- ct_blkgrps_sf %>%
  as.data.frame() %>%
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>%
  group_by(REP_DIST) %>%
  summarize(BGsHEVAC = n())

senateHEVAC <- ct_blkgrps_sf %>%
  as.data.frame() %>%
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>%
  group_by(SEN_DIST) %>%
  summarize(BGsHEVAC = n())
...
# create layer of ct_blkgrps_sf for flood and hurricane
ct_blkgrps_sf_NFHZA <- ct_blkgrps_sf %>% 
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, floodtypes) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

ct_blkgrps_sf_HEVAC <- ct_blkgrps_sf %>% 
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>% 
...
Flood and storm surge are significant risks in Connecticut. In the event of extensive inland or coastal flooding, evacuation may be required. For individuals and households with limited mobility, either due to inadequate access to transportation options or because of physical limitations, evacuation presents heightened risk. Evacuation may also prove especially difficult for individuals and households due to limited economic resources, difficulty understanding or accessing information, or low trust in official sources of information. 

Over 257,000 people live in a FEMA flood zone and over 342,000 live in a hurricane evacuation zone. Households without a car and Connecticut’s low income EJ block groups are disproportionately present in either a flood or a hurricane evacuation zone.

These interactive figures identify communities across Connecticut that are most most vulnerable to the risks of inland flooding or coastal storm surge.
...
PopupHouse <- paste0("Connecticut state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGsNFHZA,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within FEMA flood zones.")

PopupSenate <- paste0("Connecticut state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsNFHZA,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within FEMA flood zones.")

Popup <- paste0(ct_blkgrps_sf_NFHZA$NAME, "<br/>",
                "<b>State House District:</b> ", ct_blkgrps_sf_NFHZA$REP_DIST, "<br/>",
                "<b>State Senate District:</b> ", ct_blkgrps_sf_NFHZA$SEN_DIST, "<br/>",
                "<b>Town:</b> ", ct_blkgrps_sf_NFHZA$TOWN, "<br/>",
                "<b>Priority Populations: </b>", ct_blkgrps_sf_NFHZA$POPSlabel, "<br/>",
                "<b>FEMA flood risks: </b> ", ct_blkgrps_sf_NFHZA$floodtypes)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = ct_blkgrps_sf_NFHZA, 
              color = "red",
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
...
PopupHouse <- paste0("Connecticut state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGsHEVAC,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within hurricane evacuation zones.")

PopupSenate <- paste0("Connecticut state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsHEVAC,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within hurricane evacuation zones.")

Popup <- paste0(ct_blkgrps_sf_HEVAC$NAME, "<br/>",
                "<b>State House District:</b> ", ct_blkgrps_sf_HEVAC$REP_DIST, "<br/>",
                "<b>State Senate District:</b> ", ct_blkgrps_sf_HEVAC$SEN_DIST, "<br/>",
                "<b>Town:</b> ", ct_blkgrps_sf_HEVAC$TOWN, "<br/>",
                "<b>Priority Populations: </b>", ct_blkgrps_sf_HEVAC$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ct_blkgrps_sf_HEVAC, 
              color = "red",
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
...
*Note that digital data on flood risk for the northwestern and northeastern corners of Connecticut is not currently available from FEMA. Flood risk for these areas is likely to be underestimated.*

The analysis of hurricane evacuation risk presented here is based on Hurricane Evacuation Maps produced by the [U.S. Army Corps of Engineers for New England](https://www.nae.usace.army.mil/Missions/Projects-Topics/Connecticut-Hurricane-Studies/). The maps show Hurricane Evacuation Zones that are recommended to be evacuated during potential worst-case hurricane storm surge inundation. These evacuation zones are based on storm surge modeling by the National Weather Service’s National Hurricane Center using the hydrodynamic [Sea, Lake, and Overland Surges from Hurricanes (SLOSH) model](https://www.nhc.noaa.gov/nationalsurge/) to simulate storm surge from tropical cyclones. 
...
ct_FloodPops_df %>% 
  mutate(
    Status = case_when(
           ...
           PctFlood < ct_FloodPops_df[ct_FloodPops_df$Group=="Total Pop",4] ~ "Below state avg",
           PctFlood > ct_FloodPops_df[ct_FloodPops_df$Group=="Total Pop",4] ~ "Above state avg"),
  ...
  hc_title(text = "Connecticut Populations Living within Flood Zones", useHTML = TRUE)
```
...
```{r graphHEVAC, fig.align="center", fig.cap="Connecticut Populations Living within Hurricane Evacuation Zones."}
# Add status variable and recode demographic names
ct_HevacPops_df %>% 
  mutate(
    Status = case_when(
           Group == "Total Pop" ~ "State avg",
           PctHevac < ct_HevacPops_df[ct_HevacPops_df$Group=="Total Pop",4] ~ "Below state avg",
           PctHevac > ct_HevacPops_df[ct_HevacPops_df$Group=="Total Pop",4] ~ "Above state avg"),
    Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma"),
    PctHevac = round(PctHevac,2)) %>% 
  ...
  hc_xAxis(title = NULL, type = "category", labels = list(step = 1)) %>% 
  hc_tooltip(pointFormat = "{series.name}: {point.y}%") %>% 
  hc_title(text = "Connecticut Populations Living within Hurricane Evacuation Zones", useHTML = TRUE)
```
...
*Note that digital data on flood risk for most of western Connecticut is not currently available from FEMA. Flood risk for western Connecticut is likely to be underestimated.*
...
These tables show the number and percentage of the general population by jurisdiction exposed to FEMA flood risk or hurricane evacuation risk. FEMA flood risk is calculated as the percentage of a Census block group that falls within 100-year or 500-year flood risk zones and aggregated by jurisdiction. Hurricane evacuation risk is calculated as the percentage of a Census block group that falls within hurricane evacuation zones identified by the Connecticut Emergency Management Agency and U.S. Army Corps of Engineers, and aggregated by jurisdiction. Population values are derived from the American Community Survey 5-year estimates for 2014-2018.
