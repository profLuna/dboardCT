...
# Read in no transit access. Whole census units with no transit access. Includes ct_busBuff400m_sf with counts of pop with access to bus. 
...
ct_blkgrps_sf <- readRDS(file = "../DATA/ct_blkgrps_sf_CUM.Rds")

#create layer of municipalities
ct_towns <- county_subdivisions("CT") %>% 
  st_transform(., crs = 4326) %>% 
  select(NAME,NAMELSAD)

# Assign municipality names to block groups
ct_blkgrps_sf <- county_subdivisions("CT") %>% 
  st_transform(., crs = st_crs(ct_blkgrps_sf)) %>% 
  transmute(TOWN = NAME) %>% 
  st_join(ct_blkgrps_sf, ., largest = TRUE) %>% 
  mutate(NAME = str_remove_all(NAME, ", Connecticut")) %>% 
  st_transform(., crs = 4326)

# create layer of state house districts
house_districts <- st_read("../DATA/shapefiles/housect_37800_0000_2010_s100_census_1_shp/nad83","housect_37800_0000_2010_s100_census_1_shp_nad83_meters") %>% 
  transmute(REP_DIST = NAMELSAD10) %>% 
  st_transform(., crs = 4326) %>% 
  st_make_valid()

# create layer of state senate districts
senate_districts <- st_read("../DATA/shapefiles/senatect_37800_0000_2010_s100_census_1_shp/nad83","senatect_37800_0000_2010_s100_census_1_shp_nad83_meters") %>% 
  transmute(SEN_DIST = NAMELSAD10) %>% 
  st_transform(., crs = 4326) %>% 
  st_make_valid()

# Assign state house district names to block groups
ct_blkgrps_sf <- house_districts %>% 
  select(REP_DIST) %>% 
  st_join(ct_blkgrps_sf, ., largest = TRUE)

# Assign state senate district names to block groups
ct_blkgrps_sf <- senate_districts %>% 
  select(SEN_DIST) %>% 
  st_join(ct_blkgrps_sf, ., largest = TRUE)

# identify block groups with no bus access
ct_blkgrps_sf <- ct_blkgrps_sf_noBus %>% 
  as.data.frame() %>% 
...
ct_blkgrps_sf <- ct_blkgrps_sf %>% 
  mutate(Minority80th = if_else(percent_rank(minority_pctE) >= 0.8, "People of Color","NA"),
    ...
    CT_INCOME_80th = if_else(CT_INCOME == "I", "CT Low Income", "NA"),
...
muniNoBus <- ct_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(NoBusAccess == "Y" & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGsNoBus = n())

houseNoBus <- ct_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(NoBusAccess == "Y" & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGsNoBus = n())

senateNoBus <- ct_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(NoBusAccess == "Y" & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGsNoBus = n())
...
muniHwy_80th <- ct_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Headwaypctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thHwy = n())

houseHwy_80th <- ct_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Headwaypctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thHwy = n())

senateHwy_80th <- ct_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Headwaypctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thHwy = n())
...
muniTcost_80th <- ct_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Tcostpctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thTcost = n())

houseTcost_80th <- ct_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Tcostpctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thTcost = n())

senateTcost_80th <- ct_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Tcostpctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thTcost = n())
...
# create layer of ct_blkgrp_sf for 80th percentile and thresholds
ct_blkgrps_sf_NoBus <- ct_blkgrps_sf %>% 
  filter(NoBusAccess == "Y" & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

ct_blkgrps_sf_Hwy <- ct_blkgrps_sf %>% 
  filter(Headwaypctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, 
         AvgStopHeadway, Headwaypctile)%>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

ct_blkgrps_sf_Walk <- ct_blkgrps_sf %>% 
  filter(NatWalkInd < 5.76 & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, NatWalkInd, Walkpctile, 
         WalkCat) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

ct_blkgrps_sf_Tcost <- ct_blkgrps_sf %>% 
  filter(Tcostpctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, hh7_t, Tcostpctile) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

# extract values for inline insertion
H80th <- round(quantile(ct_blkgrps_sf$AvgStopHeadway, 0.8, na.rm = T),1)
Tcost80th <- round(quantile(ct_blkgrps_sf$hh7_t, 0.8, na.rm = T),1)

# clean up "ct_blkgrps_sf$",
rm(list = ls(pattern = paste("muni", "_80th", "headway80th", "_tract", "houseNoBus", "Transport", "houseWalk", "senateNoBus", "senateWalk", "lai_ct_blkgrp", "lai_ToverH", "_noTransit", "ct_blkgrps_sf_noBus", sep = "|"), ))
```
...
Despite the variety of transit services across Connecticut, 45% of the state’s residents do not have physical access to any form of public transit. Only 30% of the population lives within a reasonable walking distance of a bus stop. The places that have the least access to public transit, the least frequent service, lower than average walkability, and the highest transportation cost burdens are all the less urban parts of the state, particularly for persons over 64 years of age.
...
#   palette = "YlOrRd",
#   domain = ct_blkgrps_sf_PM25$PM25_19,
#   n = 5)
...
PopupHouse <- paste0("Connecticut state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGsNoBus,"</b>", " <b>Block Groups</b> with high percentages of priority populations and no reasonable access to a bus.")

PopupSenate <- paste0("Connecticut state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsNoBus,"</b>", " <b>Block Groups</b> with high percentages of priority populations and no reasonable access to a bus.")

Popup <- paste0(ct_blkgrps_sf_NoBus$NAME, "<br/>",
                "<b>State House District:</b> ", ct_blkgrps_sf_NoBus$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ct_blkgrps_sf_NoBus$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ct_blkgrps_sf_NoBus$TOWN, "<br/>",
                      "<b>Priority Populations: </b>", ct_blkgrps_sf_NoBus$POPSlabel)

leaflet(width = "100%") %>% 
  addProviderTiles(providers$Stamen.TonerLite) %>% 
  addPolygons(data = ct_towns_simple,
              weight = 0.7,
              opacity = 1,
              color = "gray",
              fillOpacity = 0,
  ...
  addPolygons(data = ct_blkgrps_sf_NoBus, 
              color = "red",
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
...
palHwy <- colorQuantile(
  palette = "YlOrRd",
  domain = ct_blkgrps_sf_Hwy$AvgStopHeadway,
  n = 5)
...
PopupHouse <- paste0("Connecticut state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thHwy,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing long bus headways at the 80th percentile (", H80th," minutes) or longer.", "<br/>",
...
PopupSenate <- paste0("Connecticut state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thHwy,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing long bus headways at the 80th percentile (", H80th," minutes) or longer.", "<br/>",
...
Popup <- paste0(ct_blkgrps_sf_Hwy$NAME, "<br/>",
                "<b>State House District:</b> ", ct_blkgrps_sf_Hwy$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ct_blkgrps_sf_Hwy$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ct_blkgrps_sf_Hwy$TOWN, "<br/>",
                      "<b>Avg Bus Headway (minutes):</b> ", round(ct_blkgrps_sf_Hwy$AvgStopHeadway,1), "<br/>",
         "<b>Headway percentile:</b> ", round(ct_blkgrps_sf_Hwy$Headwaypctile,0), "<br/>",
                      "<b>Priority Populations: </b>", ct_blkgrps_sf_Hwy$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ct_blkgrps_sf_Hwy, 
  ...
  addLegend(title = "Highest Avg Headways (minutes)<br>for Priority Populations", 
            pal = palHwy,
            values = ct_blkgrps_sf_Hwy$AvgStopHeadway,
            position = "bottomleft",
...
palWalk <- colorQuantile(
  palette = "YlOrRd",
  domain = ct_blkgrps_sf_Walk$NatWalkInd,
  n = 5,
  reverse = TRUE)
...
PopupHouse <- paste0("Connecticut state House District ",
...
PopupSenate <- paste0("Connecticut state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsLWalk,"</b>", " <b>Block Groups</b> with high percentages of priority populations in least walkable neighborhoods.", "<br/>",
...
Popup <- paste0(ct_blkgrps_sf_Walk$NAME, "<br/>",
                "<b>State House District:</b> ", ct_blkgrps_sf_Walk$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ct_blkgrps_sf_Walk$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ct_blkgrps_sf_Walk$TOWN, "<br/>",
                      "<b>Walkability Score:</b> ", paste0(round(ct_blkgrps_sf_Walk$NatWalkInd,1), " (", ct_blkgrps_sf_Walk$WalkCat,")"), "<br/>",
         "<b>Walkability percentile:</b> ", round(ct_blkgrps_sf_Walk$Walkpctile,0), "<br/>",
                      "<b>Priority Populations: </b>", ct_blkgrps_sf_Walk$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ct_blkgrps_sf_Walk, 
  ...
  addLegend(title = "Least Walkable Scores<br>for Priority Populations", 
            pal = palWalk,
            values = ct_blkgrps_sf_Walk$NatWalkInd,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
palTcost <- colorQuantile(
  palette = "YlOrRd",
  domain = ct_blkgrps_sf_Tcost$hh7_t,
  n = 5)
...
PopupHouse <- paste0("Connecticut state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thTcost,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing transportation cost burdens at the 80th percentile - ", Tcost80th,"% of household income - or higher.", "<br/>",
...
PopupSenate <- paste0("Connecticut state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thTcost,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing transportation cost burdens at the 80th percentile - ", Tcost80th,"% of household income - or higher.", "<br/>",
...
Popup <- paste0(ct_blkgrps_sf_Tcost$NAME, "<br/>",
                "<b>State House District:</b> ", ct_blkgrps_sf_Tcost$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ct_blkgrps_sf_Tcost$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ct_blkgrps_sf_Tcost$TOWN, "<br/>",
                      "<b>Avg Transport Cost Burden:</b> ", round(ct_blkgrps_sf_Tcost$hh7_t,1), "% of household income", "<br/>",
                      "<b>Avg Transport Cost Burden percentile: </b>", round(ct_blkgrps_sf_Tcost$Tcostpctile,1), "<br/>",
                      "<b>Priority Populations: </b>", ct_blkgrps_sf_Tcost$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ct_blkgrps_sf_Tcost, 
  ...
  addLegend(title = "Highest Transport Cost Burdens<br>(% Household Income)<br>for Priority Populations", 
            pal = palTcost,
            values = ct_blkgrps_sf_Tcost$hh7_t,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
```{r graphBusAccess, fig.align="center", fig.cap="Percentage of population groups within 1/4 mile (400 meters) of a bus stop across Connecticut."}
ct_transitAccessPops_df %>% 
  arrange(PctBus) %>% 
  mutate(PctBus = round(PctBus,1), 
         Status = case_when(
           Group == "Total Pop" ~ "State avg",
           PctBus < ct_transitAccessPops_df[1,6] ~ "Below state avg",
           PctBus > ct_transitAccessPops_df[1,6] ~ "Above state avg"),
  ...
  hc_title(text = "Connecticut Populations within Walking Distance (400m) of Bus Stops", 
           useHTML = TRUE)
```
...
```{r graphHwy, fig.align="center", fig.cap="Population-weighted average bus headways for groups within 400 meters of a bus stop across Connecticut."}
# extract total pop value
popAvgHwy <- ct_busHeadwayPops_df %>% 
...
ct_busHeadwayPops_df %>% 
  ...
  hc_title(text = "Population−Weighted Average Bus Stop Headway for Connecticut Groups within Walking Distance (400m) of Bus Stops", 
           useHTML = TRUE)
```
...
```{r graphWalk, fig.align="center", fig.cap="Population-weighted average walkability index scores for groups across Connecticut."}
popAvgWalk <- ct_wAvgWalkability_df %>% 
  filter(Group == "Total Pop") %>% 
...
```{r graphTcost, fig.align="center", fig.cap="Population-weighted average transportation cost burden as a percentage of household income for groups across Connecticut."}
# extract total pop value
popAvgTcost <- lai_ct_df %>% 
  filter(Group == "Total Pop") %>% 
  select(wTBurden) %>% 
  pull()
...
For example, adults over the age of 64 (i.e., 'Over 64') in the state reside in Census block groups that experience population-weighted average bus headways of over `r round(ct_busHeadwayPops_df %>% filter(Group == "Over 64") %>% select(AvgWHeadway) %>% pull(),1)` minutes. Compare this to the state average (i.e., `Total Pop`) of `r round(ct_busHeadwayPops_df %>% filter(Group == "Over 64") %>% select(AvgWHeadway) %>% pull(),1)` minutes on average between buses for those with access to bus stops. 
...
busAccesbyTown <- ct_busBuff400m_sf %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(st_transform(ct_towns, st_crs(ct_busBuff400m_sf))) %>% 
...
town_busAccess_pct <- ct_towns_sf %>% 
  as.data.frame() %>% 
  left_join(., town_pops, by = "GEOID") %>% 
  select(NAME, totalpopE) %>% 
  left_join(busAccesbyTown, ., by = "NAME") %>% 
  mutate(PctBusAccess = BusPop/totalpopE*100)

ct_towns %>% 
  as.data.frame() %>% 
  filter(NAME != "County subdivisions not defined") %>% 
  left_join(., town_busAccess_pct, by = "NAME") %>% 
...
busAccesbyHouse <- ct_busBuff400m_sf %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
    st_transform(house_districts,st_crs(ct_busBuff400m_sf))
    )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
house_names_pops <- ct_blkgrps_sf %>% 
  st_transform(.,st_crs(ct_busBuff400m_sf)) %>% 
  select(totalpopE) %>% 
...
# calculate bus access pops per district
busAccesbySenate <- ct_busBuff400m_sf %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
    st_transform(senate_districts,st_crs(ct_busBuff400m_sf))
    )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
